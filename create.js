const tagActiveClass = 'tagActive';
const searchHiddenClass = 'searchHidden';
const closeTagClass = 'closeTag';
const tagClass = 'tag';
...
function getTag(tag, tagClasses) {
    return `<span class="${tagClasses}">
                ${tag}
            </span>`;
}

function getJobListing(jobData, filterTags = []) {
    const jobTagPlaceholder = '###JOB_TAGS###';
    let jobListingHTML = `
        <div class="jobsItem">
            <div class="jobsColumn jobsColumnLeft">
                <img src="${jobData.logo}" alt="${jobData.company}" class="jobsImg" />
                <div class="jobsInfo">
                    <span class="jobsCompany">${jobData.company}</span>
                    <span class="jobsTitle">${jobData.position}</span>
                    
                    <ul class="jobsDetails">
                        <li class="jobsDetailsItem">${jobData.postedAt}</li>
                        <li class="jobsDetailsItem">${jobData.contract}</li>
                        <li class="jobsDetailsItem">${jobData.location}</li>
                    </ul>
                </div>
            </div>
            <div class="jobsColumn jobsColumnRight">
                ${jobTagPlaceholder}
            </div>
        </div>
    `;
    ...
    const tagsListLowercase = tagsList.map(e => e && e.toLowerCase());
    ...
    const tagsString = tagsList.reduce((acc, currentTag) => {
        ...
        return acc + getTag(currentTag, `${tagClass} ${activeClass}`);
    }, '');

    return jobListingHTML.replace(jobTagPlaceholder, tagsString);
};
...
function getSearchBar(tagValue, searchContentEl) {
...
function setJobsListings(filterTags) {
    const jobsListingsHTML = jobsListings.reduce((acc, currentListing) => {
        return acc + getJobListing(currentListing, filterTags);
    }, '');
...
function displaySearchWrapper(display = false) {
    ...
    if (display) {
        searchWrapper.classList.remove(searchHiddenClass);

        return;
    }

    searchWrapper.classList.add(searchHiddenClass);
}

function setSearchbarContent(searchContentEl, tags) {
    searchContentEl.innerHTML = tags.reduce((acc, currentTag) => {
        return acc + getTag(currentTag, closeTagClass);
    }, '');
}
...
window.addEventListener('click', (event) => {
    ...
    const searchBarTags = getSearchBar(targetText, searchContentEl);
